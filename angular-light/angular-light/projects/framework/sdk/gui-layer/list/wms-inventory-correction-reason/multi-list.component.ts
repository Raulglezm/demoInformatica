...
@Directive({
  selector: '[EdmMultiListWmsInventoryCorrectionReason]',
})
export class EdmMultiListWmsInventoryCorrectionReason implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('wms-inventory-correction-reason');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('wms-inventory-correction-reason', this.viewContainerRef).then((list) => {
