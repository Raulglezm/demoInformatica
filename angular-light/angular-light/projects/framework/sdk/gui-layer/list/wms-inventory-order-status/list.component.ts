...
@Directive({
  selector: '[EdmListWmsInventoryOrderStatus]',
})
export class EdmListWmsInventoryOrderStatus implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('wms-inventory-order-status');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('wms-inventory-order-status', this.viewContainerRef).then((list) => {
