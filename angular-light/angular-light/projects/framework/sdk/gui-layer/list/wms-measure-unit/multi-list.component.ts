...
@Directive({
  selector: '[EdmMultiListWmsMeasureUnit]',
})
export class EdmMultiListWmsMeasureUnit implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('wms-measure-unit');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('wms-measure-unit', this.viewContainerRef).then((list) => {
