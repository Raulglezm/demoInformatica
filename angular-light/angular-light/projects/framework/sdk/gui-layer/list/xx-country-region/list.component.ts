...
@Directive({
  selector: '[EdmListXxCountryRegion]',
})
export class EdmListXxCountryRegion implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-country-region');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-country-region', this.viewContainerRef).then((list) => {
