...
@Directive({
  selector: '[EdmMultiListXxCountryRegion]',
})
export class EdmMultiListXxCountryRegion implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-country-region');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-country-region', this.viewContainerRef).then((list) => {
