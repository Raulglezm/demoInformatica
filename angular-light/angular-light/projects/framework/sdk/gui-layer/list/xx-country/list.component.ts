...
@Directive({
  selector: '[EdmListXxCountry]',
})
export class EdmListXxCountry implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-country');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-country', this.viewContainerRef).then((list) => {
