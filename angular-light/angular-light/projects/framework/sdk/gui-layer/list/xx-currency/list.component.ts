...
@Directive({
  selector: '[EdmListXxCurrency]',
})
export class EdmListXxCurrency implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-currency');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-currency', this.viewContainerRef).then((list) => {
