...
@Directive({
  selector: '[EdmMultiListXxGender]',
})
export class EdmMultiListXxGender implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-gender');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-gender', this.viewContainerRef).then((list) => {
