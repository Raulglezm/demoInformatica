...
@Directive({
  selector: '[EdmMultiListXxGovernmentIdType]',
})
export class EdmMultiListXxGovernmentIdType implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-government-id-type');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-government-id-type', this.viewContainerRef).then((list) => {
