...
@Directive({
  selector: '[EdmListXxLanguage]',
})
export class EdmListXxLanguage implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-language');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-language', this.viewContainerRef).then((list) => {
