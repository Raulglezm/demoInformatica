...
@Directive({
  selector: '[EdmMultiListXxLanguage]',
})
export class EdmMultiListXxLanguage implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-language');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-language', this.viewContainerRef).then((list) => {
