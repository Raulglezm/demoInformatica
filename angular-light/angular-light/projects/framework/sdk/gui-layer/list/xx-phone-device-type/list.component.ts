...
@Directive({
  selector: '[EdmListXxPhoneDeviceType]',
})
export class EdmListXxPhoneDeviceType implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-phone-device-type');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-phone-device-type', this.viewContainerRef).then((list) => {
