...
@Directive({
  selector: '[EdmMultiListXxPhoneDeviceType]',
})
export class EdmMultiListXxPhoneDeviceType implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-phone-device-type');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-phone-device-type', this.viewContainerRef).then((list) => {
