...
@Directive({
  selector: '[EdmListXxSocialDenomination]',
})
export class EdmListXxSocialDenomination implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-social-denomination');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-social-denomination', this.viewContainerRef).then((list) => {
