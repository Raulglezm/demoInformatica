...
@Directive({
  selector: '[EdmListWmsInventoryCorrectionReason]',
})
export class EdmListWmsInventoryCorrectionReason implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('wms-inventory-correction-reason');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('wms-inventory-correction-reason', this.viewContainerRef).then((list) => {
