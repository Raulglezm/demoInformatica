...
@Directive({
  selector: '[EdmMultiListWmsInventoryOrderStatus]',
})
export class EdmMultiListWmsInventoryOrderStatus implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('wms-inventory-order-status');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('wms-inventory-order-status', this.viewContainerRef).then((list) => {
