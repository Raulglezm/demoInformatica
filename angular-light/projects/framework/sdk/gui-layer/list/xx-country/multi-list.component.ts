...
@Directive({
  selector: '[EdmMultiListXxCountry]',
})
export class EdmMultiListXxCountry implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-country');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-country', this.viewContainerRef).then((list) => {
