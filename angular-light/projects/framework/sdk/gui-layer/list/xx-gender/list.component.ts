...
@Directive({
  selector: '[EdmListXxGender]',
})
export class EdmListXxGender implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-gender');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-gender', this.viewContainerRef).then((list) => {
