...
@Directive({
  selector: '[EdmListXxGoogleBusinessType]',
})
export class EdmListXxGoogleBusinessType implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-google-business-type');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-google-business-type', this.viewContainerRef).then((list) => {
