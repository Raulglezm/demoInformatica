...
@Directive({
  selector: '[EdmMultiListXxGoogleBusinessType]',
})
export class EdmMultiListXxGoogleBusinessType implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-google-business-type');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-google-business-type', this.viewContainerRef).then((list) => {
