...
@Directive({
  selector: '[EdmListXxLanguageLevel]',
})
export class EdmListXxLanguageLevel implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadList('xx-language-level');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentList('xx-language-level', this.viewContainerRef).then((list) => {
