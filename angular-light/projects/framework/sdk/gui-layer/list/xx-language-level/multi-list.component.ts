...
@Directive({
  selector: '[EdmMultiListXxLanguageLevel]',
})
export class EdmMultiListXxLanguageLevel implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-language-level');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-language-level', this.viewContainerRef).then((list) => {
