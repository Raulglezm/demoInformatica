...
@Directive({
  selector: '[EdmMultiListXxNationality]',
})
export class EdmMultiListXxNationality implements OnInit {
  private static ready: Promise<void> = null;
  public static setReady(): Promise<void> {
    if (this.ready === null) {
      this.ready = ControllerProvider.instance.loadMultiList('xx-nationality');
    }
    return this.ready;
  };
  ...
  ngOnInit(): void {
    this.controller.createComponentMultiList('xx-nationality', this.viewContainerRef).then((list) => {
